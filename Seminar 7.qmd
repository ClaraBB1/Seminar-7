...
site_sf <- st_as_sf(site_df, coords = c("Long", "Lat")) #order of long,lat matters.

# This will load the spatial data including a CRS, since lat/long usually safe to say crs: 4326
...
# Create the POINT object, p1 is the lat and long of the first row of data
p1 <- st_point(c(x = p1_df$Long, y = p1_df$Lat))

# Show the class of this object, this one is an sfg, which is just a point, it is just geometry, the initial building block, it does not give anything on the CRS
class(p1)
```
...
# p2 is also a point, for second row of data
p2_df <- site_df |> 
  slice(2) |> 
  select(Lat, Long)
...
```{r}
# Filter site_df dataframe to get first and last rows (many ways to do this), here without the lat/long since they are store under the points
...
site_sf_filter <- st_sf(site_df_filter, pts) # the 2 points
site_sf_filter

#### this is all to show what happens in the background of creating an sf data frame
## sfg objects we will use
```
...
site_bbox
# gives me a list of points, coordinate
```
...
```{r}
library(bcdata)
# bcdc_search is function to search BCdata catalogue
# can click on tab created to see the url link needed
...
# tells us 9174 features and 19 fields, but don't want this much data on our computer
```
...
```{r}
# to transform from wgs84 to BC alberts crs
site_sf_albers <- st_transform(site_sf, 3005)
bcdc_query_geodata(gs_search[[1]]$id, crs = 3005) %>% 
  filter(BBOX(site_sf_albers))  # selecting within the bounding box
```
...
# to view where these greenspaces are
mapview(greenspaces)
```
...
# Perform the intersection (clip), want to intersect greenspaces with area of interest (aoi)
greenspaces_clip <- st_intersection(greenspaces_aoi, site_aoi_sfc)

# Observe results
nrow(greenspaces_clip) == nrow(greenspaces_aoi)
#to see that the clipping worked as expected
mapview(site_aoi, col.regions = "red") + mapview(greenspaces_clip)

# to see the total area of the clip and aoir
...
# to see what we are downloading
...
1.  Specifying the dataset that you want to download (the permanent link)
...
```{r}
# package that provide a single line of codes
library(bcmaps)

# Explore all available layers
View(available_layers())
# we will focus on the first 2 columns that shows all the layers
...
Some functions, like `cded_terra()`, (Canadian Digital Elevation data using the terra package) are worth viewing their help page for. This function will download DEM ***raster*** data (which is more data dense) instead of vector data, so it's important to specify an AOI to download that data, rather than trying to download an entire province worth of data. For example, let's download the DEM that falls within our AOI we created earlier (NOTE: I use plot commands throughout instead of the mapview functions because plotting is much faster to complete by comparison):

```{r}
library(terra)

# to get all the elavation data within area of interest
site_dem <- cded_terra(site_aoi) # cded_terra from the BCmap package
site_dem
plot(site_dem) # is faster then mapview because we are starting to have a lot of data
...
# ext is the extent, resample at resolution of 20m
...
# it was not the same shape as area of interest, so have to mask 
